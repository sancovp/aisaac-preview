...
Each slot has a spectrum (the parent's children) with a finite size
(`getSpectrumSizeAtLevel`, `kernel-v2.ts:223-256`), and `0` at a slot means "this slot
doesn't discriminate" — uniform over that slot's spectrum (`kernel-v2.ts:135-143`).

The independence we need — that positions do not constrain each other's *committability* —
is a **structural** fact about the encoding's commitment operations, not a fact about the
kernel. (The kernel *does* factor per slot, `K(x,y) = ∏_k K_k(x_k, y_k)`,
`kernel-v2.ts:9`, `kernel-v2.ts:150-217`, but that factorization is the wrong evidence: a
product *kernel* decomposes a *similarity function* over already-valid coordinates, and is
logically independent of which *joint commitments are admissible* in the carrier. It does
not entail independence; it is irrelevant to it.) The decisive evidence is the encoding's
own structure:

- **No operation couples two positions.** Every commitment operation is per-node and gates
  only on that node's own local state: `lockNode` commits a node's spectrum and its only
  precondition is that node's own `node.children.length < 2 || node.terminal`
  (`index.ts:1655-1670`); `resolve`/`scry`/`instantiate` each commit one value into one
  slot at a time; the Griess `verifyKappa` admissibility check is a per-invariant *monotone
  presence* check, never a cross-position exclusion. There is simply no construct that can
  make committing position *p* forbid a still-uncommitted position *q*.
- **The configuration count is the full product.** `mineConfigurations` computes
  `totalConfigs = ∏ (per-dimension child counts)` (`mine.ts:733`) and enumerates
  configurations by varying *each dimension independently* (`mine.ts:764-790`). The carrier
  is therefore the **free cartesian product** of the per-position spectra: were any joint
  commitment forbidden, the count would be strictly *less* than the product. The product
  count is itself the proof that all joint commitments are admissible.

So positions are **independent coordinates of a product structure**: a configuration is a
choice (or non-choice `0`) at each position, and positions do not constrain each other's
committability (drilling opens a position's sub-space, but that only adds positions; it
never makes two sibling commitments mutually exclusive).
...
## Part 7 — Scope boundary: what this result covers, and what it does NOT

This result is about Crystal Ball's **encoding** — the *syntactic carrier*. Precisely: the
carrier `D` is the set of all configurations whose only constraints are **functional** (no
position holds two values) and **tree-respecting** (a deep position's governing shallower
position is committed). By Change-1's structural argument that carrier is the **free
cartesian product** of the per-position spectra — every joint commitment is admissible,
the count is the full product (`mine.ts:733`), no operation couples two positions. *On that
carrier* Invariant E holds and the four Scott-domain axioms (Part 4) are proved. Nothing
above this section is conditioned on anything beyond functional + tree-respecting
commitment; the proof stands exactly as written, for the encoding.

**Consistency / admissibility / "meaning" is NOT adjudicated by the encoding.** It is
adjudicated by an **external reasoner** — the `youknow()` compiler, invoked
*out-of-process* (the bridge shells out via `execSync`/docker exec, `youknow-bridge.ts`),
and fed **one per-node UARL statement at a time** (`engine.ts:1205-1208` loops
`session.youknowFn(uarl.raw)` over `buildSpaceUARL`'s output; `buildUARL` is per-node). The
encoding **never constructs a joint-configuration admissibility query** — it asks only
per-node questions — so it imposes **no inter-position coupling**. Verdict on the
encoding's role: **EXTERNAL.** This is exactly why the carrier stays the free product and
the proof goes through: the thing that *could* couple positions lives outside the encoding.

**Therefore the result does NOT automatically transfer to the externally-consistent
subspace.** Consider `D_consistent ⊆ D` = the configurations the `youknow()` validators
accept as consistent/meaningful. The validators **can** couple positions (an admissibility
judgement may depend on several positions jointly). If the admissibility predicate is **not
downward-closed**, bounded-completeness can fail on `D_consistent`: there could be two
admissible configurations σ, τ with an admissible common upper bound (so they are "bounded"
in `D_consistent`) whose union `σ ⊔ τ` is *inadmissible* — i.e. a bounded pair with no
least upper bound *inside the consistent subspace*. Such a coupling is impossible in the
encoding (Part 6.1, Change 1) but is entirely possible for an external reasoner that
inspects joint configurations.

Whether `D_consistent` — the consistency-constrained object — is *itself* a Scott domain is
a **separate, OPEN proposition**, not established here. It would require (at least) showing
the external admissibility predicate is downward-closed (or otherwise lub-closed on bounded
families). **This proof's verdict (PROVED) covers the syntactic encoding only; it must not
be read as covering the consistency-constrained / externally-validated object.**
