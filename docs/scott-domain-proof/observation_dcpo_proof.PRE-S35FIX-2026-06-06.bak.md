...
**$R$ is a growing operator.** Given a finite schema, $R$ wraps each slot's children in their **orbit
decomposition** as *new* structure. At each slot, the children are grouped by structural isomorphism —
two children are in the same group iff their subtrees have the same **kind** (the same arity-and-opening
structure, recursively, per Section 2.2) — and each group is recorded as an **orbit** and inserted beneath
the slot as an added layer. Crucially, $R$ does **not** collapse multiplicity: $k$ structurally-parallel
children remain $k$ children, now organized under the orbit they belong to; the description *grows*. (A
decomposition that instead replaced the children by one representative per kind would trivialize the
analysis — every schema would close immediately — and is not the operator under study.)
...
> same type, $R$ can be applied again, and the iteration $K, R(K), R^2(K), \dots$ is well-typed.
...
**The obstruction count $\Omega$.** The orbit grouping computed slot-by-slot is *local*: it groups children
by their kind (structural isomorphism of subtrees). This local grouping need not coincide with the grouping
induced by the schema's *global* symmetry. Let $\mathrm{Aut}(K)$ be the automorphism group of $K$ as a
finite tree — the structure-preserving and identity-preserving bijections of its nodes (two
structurally-identical children that are nonetheless *distinct, distinguishable nodes* are interchangeable
only if some automorphism of the whole schema maps one to the other). At each slot $s$, $\mathrm{Aut}(K)$
induces a partition of $s$'s children into **global orbits**. Define
$$\Omega(K) = \#\{\, \text{slots } s : \text{the local structural-isomorphism partition at } s \ \neq\ \text{the global } \mathrm{Aut}(K)\text{-orbit partition at } s \,\}.$$
Then $\Omega(K) = 0$ exactly when, at every slot, "locally same-kind children" coincides with "globally
symmetric children" — i.e. every local symmetry *lifts* to a global automorphism with no residue. Because
$K$ is finite, $\mathrm{Aut}(K)$ and both partitions are computable, so $\Omega$ is a computable function
of $K$.

> **Definition (Crowning).** A finite schema $K$ **crowns** iff (i) reifying it reproduces its own canonical
> form — $\mathrm{canon}(R(K)) = \mathrm{canon}(K)$, equivalently $R(K) \cong K$, a **one-step fixed
> point** — **and** (ii) it is obstruction-free, $\Omega(K) = 0$. A crowned schema is one that reproduces
> its own description under self-abstraction with no residual defect: it has closed cleanly into itself.

> **Proposition 5 (Crowning is decidable in one step).** For each finite schema $K$, whether $K$ crowns is
> decidable.

*Proof.* Compute $R(K)$, compare $\mathrm{canon}(R(K))$ to $\mathrm{canon}(K)$, and check $\Omega(K) = 0$.
Each is a finite computation on a finite schema: $R$ is computable (type-closure above); canonical forms of
finite trees are computable and comparable; $\mathrm{Aut}(K)$ and the two partitions are computable.
Crowning is the conjunction of an equality test and a zero test, so it is decided in finitely many
steps. $\qquad\blacksquare$
...
1. **Crown** — $\mathrm{canon}(R(K)) = \mathrm{canon}(K)$ and $\Omega(K) = 0$. *Witness:* a root with $N$
   structurally-identical leaf children (a single slot, one orbit of size $N$). $R$ groups the $N$ identical
   leaves into one symmetric orbit and reproduces the same canonical form, and the local symmetry (the full
   symmetric group on the $N$ leaves) *is* the global automorphism group, so $\Omega = 0$. The schema
   crowns. Robust across $N = 4, 7, 12$.
2. **Stable-but-obstructed** — $\mathrm{canon}(R(K)) = \mathrm{canon}(K)$ yet $\Omega(K) > 0$. *Witness:* a
   root with $k$ children that are structurally identical (same kind) but mutually **distinct, distinguishable
   nodes** — each a separately-identified element no automorphism can interchange. Reification produces a
   stable canonical form (one orbit of size $k$, reproduced under further reification), so the signature is a
   one-step fixed point — yet the local $k$-fold symmetry does **not** lift to a global automorphism (the
   identities forbid the permutation), so the local and global partitions at the root disagree and
   $\Omega = 1 > 0$. The schema does **not** crown, despite a stable signature. *This is the witness that
   makes $\Omega$ meaningful: there exist stable-signature schemas the criterion correctly refuses to
   crown.* It is exhibited for $k = 3$ and $k = 6$, so it is a general pattern, not a one-off.
3. **No closure (halt)** — reification yields a node that cannot be closed. *Witness:* any genuinely
   multi-slot schema (depth $\ge 2$, or a single slot whose orbits themselves carry sub-structure).
   Wrapping a single-orbit slot produces a node with exactly one child, which the closing discipline forbids
   (a node that closes must offer at least two alternatives); the closing act therefore halts without a
   clean fixed point. The schema does **not** crown.

Hence **"not every schema crowns" is honestly true**: outcomes 2 and 3 are genuine non-crown closures, each
realized by a concrete minimal schema. The value of the criterion is *epistemic* — it does not promise that
structures crown; it **decides**, for each, whether it has closed cleanly. The obstruction gate
$\Omega = 0$ is part of that decision, not a defect: a stable signature carrying an unlifted local symmetry
(outcome 2) is correctly judged not-yet-clean.

> **Proposition 6 (Depth-1 schemas crown).** A schema consisting of a single root slot with finitely many
> structurally-identical children (a depth-$1$ tree — the crown witness) crowns: the local symmetric-group
> symmetry on the leaves is exactly the global automorphism group (an automorphism of a depth-$1$ tree may
> permute identical leaves and nothing else), so $\Omega = 0$, and reification reproduces the canonical
> form, so $\mathrm{canon}(R(K)) = \mathrm{canon}(K)$.

*Proof.* At depth $1$, two children are locally grouped iff they are the same kind of leaf, and they lie in
the same $\mathrm{Aut}(K)$-orbit iff some automorphism swaps them — which, at depth $1$ with no further
distinguishing structure, holds exactly for same-kind leaves. So the two partitions coincide and
$\Omega = 0$. Reifying $N$ identical leaves yields one orbit of size $N$ under the root, of the same
canonical form, so $\mathrm{canon}(R(K)) = \mathrm{canon}(K)$. Hence $K$ crowns. $\qquad\blacksquare$
...
asymmetric, deep, and the distinctly-identified cases above): single-slot uniform schemas are idempotent at
the first step, and multi-slot schemas quickly produce an unclosable node and halt. We do **not** claim a
proven universal bound on the number of growing steps, and we found no unbounded-growth witness; a universal
bound is plausible but is left as a residual obligation, not asserted. This is exactly consistent with $R$
being a terminal, closing operation: the closure is declared, not proven to be reached by descent, so the
absence of a descent bound is the expected character of the operator, not a gap in the result.
...
its own shape, $\Omega = 0$). The comparison the backdrop affords — "given these orbits, supply more in the
...
3. **Check fit and closure.** Read the result (the decode map) and test it against the backdrop: does it
   ...
   ($\Omega = 0$)? The crowning criterion (Proposition 5) is a one-step, decidable check.
...
coincides with the obstruction-free subspace $\{\Omega = 0\}$. The second conjecture is a *substantive
bridge*, not a definition: $\Omega$ is defined on schemas via the symmetry of the orbit decomposition
(Section 3.5), whereas fold-freeness is defined on configurations via injectivity of the read/round-trip map
