...
of the real line. Within each level of a coordinate, a position carries either the marker `0` — a
...
class-versus-instance digit structure and its drill nesting — and nothing more. `coordToReal` is shown to be a
faithful address of each coordinate into the real line (Section 3.5); it is the specialization order, not the
...
expansions; it is a **faithful (injective) address** of the carrier into $[0,1)$, which is the precise sense of
