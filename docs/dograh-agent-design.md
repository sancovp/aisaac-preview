# Dograh AI Demo Agent — Design Doc

Last updated: 2026-05-16

## Platform
**Dograh** (open source, self-hosted, BSD 2-Clause) — replaces Dograh.
- Running at: localhost:8000 (API) / localhost:3010 (UI) / Cloudflare tunnel for public access
- LLM: MiniMax-M2.7-highspeed via OpenAI-compatible base_url
- Telephony: Twilio (when ready for phone), WebRTC for browser testing
- No per-minute SaaS markup — just raw telephony + LLM token costs
...
## Trigger
- Qualified lead clicks "Get a Demo" / "Talk to our AI" on the site
- Lead has already self-qualified via the index survey (business owner, $10K+/mo revenue)
- Context from survey answers is passed to the agent via Dograh workflow variables (revenue tier, bottleneck, timeline, selected offer)
...
- Agent triggers SMS via Twilio/Dograh webhook → Stripe checkout link
...
## Tools the Agent Needs (Dograh function calling)
...
| `transfer_to_isaac` | Live transfer if Isaac is available | Dograh transfer action |

## Dograh Configuration

```
Platform: Dograh (vapi.ai)
Model: GPT-4o or Claude (Dograh supports both)
Voice: Professional male or female (TBD — test both)
First message: Dynamic based on survey context
Cost: ~$0.13-0.31/min (Dograh pricing)
...
Site survey → localStorage
  → "Get a Demo" click
    → Dograh widget loads with context payload:
      {
...
## Nurture System (separate from Dograh, triggered by it)
...
1. [ ] Create Dograh account + configure agent
...
6. [ ] Wire site "Get a Demo" button to Dograh widget
